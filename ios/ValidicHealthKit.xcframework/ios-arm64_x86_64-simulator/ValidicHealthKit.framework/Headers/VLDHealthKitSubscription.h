...
typedef NS_ENUM(NSUInteger, VLDHealthKitSubscriptionSet) {
    ...
    /** Includes Step Count, Distance Walking/Running, Flights Climbed, Active Energy Burned, Basal Energy Burned,
     Apple Stand Hours (from the Apple Watch), Apple Exercise Minutes (from the Apple Watch), and 
     Mindful Minutes (from the Apple Watch).
     in iOS 10 and later this will also include Wheelchair distance and Wheelchair push count. */
    ...
    /** Includes Systolic Blood Presure, Diastolic Blood Pressure, Heart Rate,
     Body Temperature, and SpO2 (oxygen saturation). */
...
typedef NS_ENUM(NSUInteger, VLDHealthKitHistoricalSet) {
    ...
    /** Indicates a historical set that generates intraday records */
    VLDHealthKitHistoricalSetIntraday
};
